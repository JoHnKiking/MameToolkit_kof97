...
class Kof97Environment(object):
    ...
    def wait_for_fight_start(self):
        ...
        while data["playing"] != 32:
            ...
            if code_160_cnt > 200:
                print("Saving.....")
                save_dir = f"/home/workspace/KOF97Saves/CH{data['2P_CH']}"
                os.makedirs(save_dir, exist_ok=True)
                res = self.emu.console.writeln(f'manager:machine():save("/home/workspace/KOF97Saves/CH{data["2P_CH"]}/kof97save_CH_{save_id}")')
                data = self.emu.step([])
            else:
                if i % 2 == 1 :
                    data = self.emu.step([Actions.P1_LEFT.value])
