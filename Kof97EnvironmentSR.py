...
class Kof98Environment(gym.Env):
    ...
    def __init__(self,roms_path = "roms/", basic_saving_dir = "/home/workspace/KOF97Saves", CH = "CH26",rewardCalculator = ComboRewardCalculatorV3(distance_rate=0.01,time_rate=0.01,damage_rate_1p=1,damage_rate_2p=1.3),env_param={}):


        render = env_param.get("render", True)
        ...
        print("env_id:"+env_id)
        self.emu = Emulator(env_id, roms_path, "kof97", self.setup_memory_addresses(), debug=True, frame_ratio=2, **env_param)
        ...
        # print("save_files: " + self.save_files)


    def reset(self):
        path = join(self.full_saving_path,random.choice(self.save_files))
        print("path: " + f'manager:machine():load("{path}")')
...
class Kof98EnvironmentV2(Kof98Environment):
    ...
    def __init__(self, roms_path = "roms/", basic_saving_dir = "/home/workspace/KOF97Saves", CH = "CH_bsa", reset_delay = 0, rewardCalculator = ComboRewardCalculatorV3(distance_rate=0.01, time_rate=0.01, damage_rate_1p=1, damage_rate_2p=1.3), env_param={}):
    ...
    def preprocess(self,data):
        ...
        # 1P 数据预处理
        observation = np.append(observation,[data["healthP1"]/103])         # 血量
        observation = np.append(observation,[data["1P_x"]/736])             # x坐标
        observation = np.append(observation,[data["1P_y"]/120])             # y坐标
        observation = np.append(observation,[data["1P_pow_value"]/128])     # 能量值

        # 解析 1P 的能量状态
        pow_state_1p = data["1P_pow_state"]
        state, pow_num = divmod(pow_state_1p,16)    # 分解为状态标志和能量槽数量
        observation = np.append(observation,[0 if state == 0 else 1])
        observation = np.append(observation,[0 if pow_num == 0 else 1])
        observation = np.append(observation,[0 if data["1P_combo"] == 0 else 1])
        # 计算 1P 速度
        ...
        # 动作序列记录
        # 将P1当前动作转为512维one-hot向量（*255可能用于增强特征）
        ...
        # 返回多模态观测数据
        return {
            "vec" : observation,            # 数值特征向量
            "act_1p": np.expand_dims(np.array(self.p1_act_seque), axis=2),  # 1P 动作序列
            "act_2p": np.expand_dims(np.array(self.p2_act_seque),axis=2),   # 2P 动作序列
            "inputs": np.expand_dims(np.array(self.process_action_sequence(data)), axis=2), # 其他动作特征
        }
    ...
    def get_status(self):
        return {
            "win_count": self.rewardCalculator.win_count,
            "loss_count": self.rewardCalculator.loss_count,
            "episode_count": self.rewardCalculator.episode_count
        }
...
class Kof98EnvironmentLSTM(Kof98Environment):
    ...
    def __init__(self,roms_path = "roms/", basic_saving_dir = "/home/workspace/KOF97Saves", CH = "CH0",rewardCalculator = ComboRewardCalculatorV3(distance_rate=0.01,time_rate=0.01,damage_rate_1p=1,damage_rate_2p=1.3),env_param={}):
